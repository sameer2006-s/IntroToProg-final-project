    ...
    for (int i = 0; i < borrow_count; i++) {
        if (borrow_book_ids[i] == book_id) {
            for (int j = i + 1; j < borrow_count; j++) {
                if (borrow_book_ids[j] == book_id &&
                    strcmp(borrow_dates[i], borrow_dates[j]) == 0) {


                    int already_printed = 0;
                    for (int k = 0; k < printed_count; k++) {
                        if (printed[k] == borrow_member_ids[i]) {
                            already_printed = 1;
                            break;
                        }
                    }
                    if (!already_printed) {
                        printf("%d\n", borrow_member_ids[i]);
                        printed[printed_count++] = borrow_member_ids[i];
                    }


                    already_printed = 0;
                    for (int k = 0; k < printed_count; k++) {
                        if (printed[k] == borrow_member_ids[j]) {
                            already_printed = 1;
                            break;
                        }
                    }
                    if (!already_printed) {
                        printf("%d\n", borrow_member_ids[j]);
                        printed[printed_count++] = borrow_member_ids[j];
                    }
